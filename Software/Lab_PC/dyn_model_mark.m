...
%% regolatore phi PID e PP  %%
pole(syst_cl(1))   % see how many unstable poles
syst_cl(1)
% see the model Properties of the G_ol(1) si extract the num and den for
% Simulink block
num_theta = cell2mat(syst_cl.Numerator(1,1))
den_theta = cell2mat(syst_cl.Denominator(1,1))

opts = pidtuneOptions('PhaseMargin',90) ;
R_theta = pidtune(delay*syst_cl(1), 'PID', 1 , opts) 
%L= R_theta*G_ol(1);
figure
%step(L/(1+L))
step(feedback(R_theta*syst_cl(1), 1))  % compact form
% impulse(feedback(R_theta*G_ol(1), 1))     %impulse
Kp_theta = R_theta.Kp ;
Ki_theta = R_theta.Ki ;
Kd_theta = R_theta.Kd ;
