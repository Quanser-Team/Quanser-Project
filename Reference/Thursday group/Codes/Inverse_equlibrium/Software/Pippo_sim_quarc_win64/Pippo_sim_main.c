...
typedef RT_MODEL_Pippo_sim_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Pippo_sim(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Pippo_sim_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(Pippo_sim_DW.HILReadEncoderTimebase2_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Pippo_sim:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Pippo_sim -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(Pippo_sim_DW.HILReadEncoderTimebase2_Task,
        (t_clock) Pippo_sim_P.HILReadEncoderTimebase2_Clock, 500.0, -1)
        ;
      if (result == 0) {
