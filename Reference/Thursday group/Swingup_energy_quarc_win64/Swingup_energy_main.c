...
typedef RT_MODEL_Swingup_energy_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Swingup_energy(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Swingup_energy_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(Swingup_energy_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Swingup_energy:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Swingup_energy -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(Swingup_energy_DW.HILReadEncoderTimebase_Task,
        (t_clock) Swingup_energy_P.HILReadEncoderTimebase_Clock, 2000.0, -1)
        ;
      if (result == 0) {
