...
typedef RT_MODEL_Test_PID_ok_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Test_PID_ok(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Test_PID_ok_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(Test_PID_ok_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Test_PID_ok:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Test_PID_ok -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(Test_PID_ok_DW.HILReadEncoderTimebase_Task,
        (t_clock) Test_PID_ok_P.HILReadEncoderTimebase_Clock, 2000.0, -1)
        ;
      if (result == 0) {
