...
void Test_PID_output(void)
{
  ...
  if (rtmIsMajorTimeStep(Test_PID_M)) {
    ...
    /* S-Function Block: Test_PID/HIL Read Encoder Timebase (hil_read_encoder_timebase_block) */
    {
      ...
      if (result < 0) {
        ...
        rtb_DiscreteTimeIntegrator = Test_PID_DW.HILReadEncoderTimebase_Buffer;
      }
    }
  }

  /* Step: '<Root>/Step1' */
  if (Test_PID_M->Timing.t[0] < Test_PID_P.Step1_Time) {
    rtb_Gain1 = Test_PID_P.Step1_Y0;
  } else {
    rtb_Gain1 = Test_PID_P.Step1_YFinal;
  }

  /* Gain: '<S1>/Gain1' incorporates:
   *  Step: '<Root>/Step1'
   */
  rtb_Gain1 *= Test_PID_P.Gain1_Gain;
  if (rtmIsMajorTimeStep(Test_PID_M)) {
    /* Gain: '<Root>/counts to rad' */
    Test_PID_B.countstorad = Test_PID_P.countstorad_Gain *
      rtb_DiscreteTimeIntegrator;
  }

  /* Sum: '<Root>/Sum' */
  rtb_Sum_h = rtb_Gain1 - Test_PID_B.countstorad;
   ...
   *  Sum: '<S32>/SumD'
   */
  Test_PID_B.FilterCoefficient = (Test_PID_P.PIDController_D * rtb_Sum_h -
   ...
   *  Sum: '<S46>/Sum'
   */
  if (Test_PID_P.ManualSwitch_CurrentSetting == 1) {
    rtb_TransferFcn = (Test_PID_P.PIDController_P * rtb_Sum_h +
                       Test_PID_X.Integrator_CSTATE) +
      Test_PID_B.FilterCoefficient;
  } else {
    rtb_TransferFcn = Test_PID_P.Constant_Value;
  }

  /* End of ManualSwitch: '<Root>/Manual Switch' */

  /* Saturate: '<Root>/Saturation' */
  if (rtb_TransferFcn > Test_PID_P.Saturation_UpperSat) {
    /* Saturate: '<Root>/Saturation' */
    Test_PID_B.Saturation = Test_PID_P.Saturation_UpperSat;
  } else if (rtb_TransferFcn < Test_PID_P.Saturation_LowerSat) {
    /* Saturate: '<Root>/Saturation' */
    Test_PID_B.Saturation = Test_PID_P.Saturation_LowerSat;
  } else {
    /* Saturate: '<Root>/Saturation' */
    Test_PID_B.Saturation = rtb_TransferFcn;
  }
  ...
  Test_PID_B.Gain_f[1] = Test_PID_P.Gain_Gain_a * Test_PID_B.countstorad;
  if (rtmIsMajorTimeStep(Test_PID_M)) {
    /* Abs: '<S54>/Abs' */
  ...
  if (rtmIsMajorTimeStep(Test_PID_M)) {
     ...
     */
    if (Test_PID_B.RelationalOperator2 != Test_PID_P.Constant_Value_j) {
  ...
  Test_PID_B.IntegralGain = Test_PID_P.PIDController_I * rtb_Sum_h;
}
...
void Test_PID_initialize(void)
{
  ...
  /* S-Function Block: Test_PID/HIL Initialize (hil_initialize_block) */
  {
    ...
    is_switching = false;
    result = hil_set_card_specific_options(Test_PID_DW.HILInitialize_Card, " ",
      2);
    if (result < 0) {
      msg_get_error_messageA(NULL, result, _rt_error_message, sizeof
        (_rt_error_message));
  ...
  Test_PID_X.Integrator_CSTATE = Test_PID_P.PIDController_InitialConditio_d;
...
RT_MODEL_Test_PID_T *Test_PID(void)
{
  ...
  /* External mode info */
  Test_PID_M->Sizes.checksums[0] = (3967154158U);
  Test_PID_M->Sizes.checksums[1] = (3423293967U);
  Test_PID_M->Sizes.checksums[2] = (3611344836U);
  Test_PID_M->Sizes.checksums[3] = (3321963014U);
  ...
  /* Initialize Sizes */
  Test_PID_M->Sizes.numContStates = (3);/* Number of continuous states */
  ...
  Test_PID_M->Sizes.numBlocks = (44);  /* Number of blocks */
  Test_PID_M->Sizes.numBlockIO = (11); /* Number of block outputs */
  Test_PID_M->Sizes.numBlockPrms = (105);/* Sum of parameter "widths" */
  return Test_PID_M;
}
