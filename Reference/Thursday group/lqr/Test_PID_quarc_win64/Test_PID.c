...
const real_T Test_PID_RGND = 0.0;      /* real_T ground */
...
void Test_PID_output(void)
{
  ...
  if (rtmIsMajorTimeStep(Test_PID_M)) {
    ...
    /* S-Function Block: Test_PID/Smooth Signal Generator1 (smooth_signal_generator_block) */
    {
      ...
      rtb_DiscreteTimeIntegrator = Test_PID_DW.SmoothSignalGenerator1_RWORK.Amp *
        y_sin;
     ...
     */
    rtb_DiscreteTimeIntegrator = Test_PID_P.Amplitudedeg_Gain *
      rtb_DiscreteTimeIntegrator * Test_PID_P.Gain1_Gain;

    /* Gain: '<S2>/Gain' incorporates:
     *  Gain: '<Root>/counts to rad'
     */
    Test_PID_B.Gain[0] = Test_PID_P.Gain_Gain * rtb_DiscreteTimeIntegrator;
    Test_PID_B.Gain[1] = Test_PID_P.countstorad_Gain * rtb_countstorad *
      Test_PID_P.Gain_Gain;

    /* Saturate: '<Root>/Saturation' */
    if (Test_PID_P.Saturation_UpperSat < 0.0) {
      /* Saturate: '<Root>/Saturation' */
      Test_PID_B.Saturation = Test_PID_P.Saturation_UpperSat;
    } else if (Test_PID_P.Saturation_LowerSat > 0.0) {
      /* Saturate: '<Root>/Saturation' */
    ...
    /* Gain: '<S3>/Gain' */
    Test_PID_B.Gain_n = Test_PID_P.Gain_Gain_k * Test_PID_B.countstorad1;
  ...
  if (rtmIsMajorTimeStep(Test_PID_M)) {
    ...
    /* S-Function Block: Test_PID/HIL Write Analog (hil_write_analog_block) */
    {
      ...
      result = hil_write_analog(Test_PID_DW.HILInitialize_Card,
        &Test_PID_P.HILWriteAnalog_channels, 1, ((const real_T*) &Test_PID_RGND));
      if (result < 0) {
        msg_get_error_messageA(NULL, result, _rt_error_message, sizeof
          (_rt_error_message));
        rtmSetErrorStatus(Test_PID_M, _rt_error_message);
...
RT_MODEL_Test_PID_T *Test_PID(void)
{
  ...
  /* External mode info */
  Test_PID_M->Sizes.checksums[0] = (3691047673U);
  Test_PID_M->Sizes.checksums[1] = (497202606U);
  Test_PID_M->Sizes.checksums[2] = (738221825U);
  Test_PID_M->Sizes.checksums[3] = (1462691517U);
  ...
  {
    Test_PID_B.Gain[0] = 0.0;
    Test_PID_B.Gain[1] = 0.0;
    Test_PID_B.Saturation = 0.0;
    Test_PID_B.countstorad1 = 0.0;
    Test_PID_B.Gain_n = 0.0;
  ...
  /* Initialize Sizes */
  Test_PID_M->Sizes.numContStates = (1);/* Number of continuous states */
  ...
  Test_PID_M->Sizes.numBlocks = (34);  /* Number of blocks */
  Test_PID_M->Sizes.numBlockIO = (8);  /* Number of block outputs */
  Test_PID_M->Sizes.numBlockPrms = (98);/* Sum of parameter "widths" */
  return Test_PID_M;
}
