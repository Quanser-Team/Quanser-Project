 ...
 * External functions *
 *====================*/
EXTERN RT_MODEL * qube_servo2_quick_start_gruppoP(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(qube_servo2_quick_start_grup_M, GBLbuf.message);
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://qube_servo2_quick_start_gruppoP:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: qube_servo2_quick_start_gruppoP -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
   ...
   ************************/
  S = qube_servo2_quick_start_gruppoP();
  if (rtmGetErrorStatus(S) != NULL) {
