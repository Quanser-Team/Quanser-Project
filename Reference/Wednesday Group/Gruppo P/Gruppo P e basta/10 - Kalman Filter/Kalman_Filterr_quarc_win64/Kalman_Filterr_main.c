...
typedef RT_MODEL_Kalman_Filterr_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Kalman_Filterr(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Kalman_Filterr_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(Kalman_Filterr_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Kalman_Filterr:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Kalman_Filterr -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      if (subrate_index < 0) {         /* all threads successfully created */
        ...
        fflush(stdout);
        result = hil_task_start(Kalman_Filterr_DW.HILReadEncoderTimebase_Task,
          (t_clock) Kalman_Filterr_P.HILReadEncoderTimebase_Clock, 500.0, -1)
          ;
        if (result == 0) {
