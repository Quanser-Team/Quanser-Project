 ...
 * External functions *
 *====================*/
EXTERN RT_MODEL * qube_servo2_quick_start(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(qube_servo2_quick_start_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(qube_servo2_quick_start_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv,
    "shmem://qube_servo2_quick_start:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: qube_servo2_quick_start -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      result = hil_task_start
        (qube_servo2_quick_start_DW.HILReadEncoderTimebase_Task, (t_clock)
         qube_servo2_quick_start_P.HILReadEncoderTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
