...
typedef RT_MODEL_Prova_sistema_T RT_MODEL;

/*====================*
 * External functions *
 *====================*/
EXTERN RT_MODEL * Prova_sistema(void);
  ...
  _do_assertion(const char * expression, const char * file_name, int line_number)
{
  ...
  rtmSetErrorStatus(Prova_sistema_M, GBLbuf.message);
}
  ...
  control_c_handler(int signal_number)
{
  ...
  GBLbuf.stopExecutionFlag = 1;
  rtExtModeStart();
  hil_task_stop(Prova_sistema_DW.HILReadEncoderTimebase_Task)
    ;
}
  ...
  main(int argc, char * argv[])
{
  ...
  rtExtModeQuarcParseArgs(argc, (const char **) argv, "shmem://Prova_sistema:1");
  ...
  if (parseError) {
    (void) fprintf(stderr,
                   "\nUsage: Prova_sistema -option1 val1 -option2 val2 -option3 ...\n\n");
    (void) fprintf(stderr,
  ...
  if (rtExtModeQuarcStartup(rtmGetRTWExtModeInfo(S),
    ...
    if (rtmGetErrorStatus(S) == NULL) {
      ...
      fflush(stdout);
      result = hil_task_start(Prova_sistema_DW.HILReadEncoderTimebase_Task,
        (t_clock) Prova_sistema_P.HILReadEncoderTimebase_Clock, 500.0, -1)
        ;
      if (result == 0) {
